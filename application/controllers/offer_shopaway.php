...
class offer_shopaway extends CI_Controller {
	...
	public function index($params=0,$page=0){
		$this->load->library('form_validation');


		if(empty($_POST) && empty($params)){
			
			$this->outputData['data'] = $this->i->getOfferIngredients($page,$params);
		}else {
			if(empty($params))
			{
		...
		//$this->outputData['recommendations'] = $this->i->getRecommended($params);
		...
		$this->load->view('offer_shopaway',$this->outputData);
	}
}