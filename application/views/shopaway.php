				    ...
				    <div class="recepie_main_left_top">
					    <div class="recepie_main_left_top_top heading">
 						...
 						<div class="threeselect">
                        	<form method="post" id="shopaway" action="<?php echo site_url('shopaway');?>">
							<div class="veg">
...
<a href="#" class="shop_way_rating_a add" data-product-id="<?php echo $datum['id'];?>" ><?php echo isProductInCart($datum['id']) ? 'In Cart' :'Add to Cart';?></a>									
						...
						<div class="pagi">
                        	...
                        	<?php 
							/*
							$pagi_tot_trows=$this->pagination->total_rows;
							$pagi_url=str_replace("//i","/i",$this->pagination->base_url);
							$pagi_perpage=$this->pagination->per_page;
							
							if($pagi_tot_trows>0)
							{
								$incval=1;
								$nclms=ceil($pagi_tot_trows/$pagi_perpage);
								if($nclms<=50) $mulval=10;
								else if($nclms<=100) $mulval=25;
								else if($nclms<=500) $mulval=100;
								else if($nclms<=1000) $mulval=250;
								$numval=floor($nclms/$mulval);
							*/	
							?>
                             <!--   <div class="pagr">
                                    <?php 
									/*for($cnv=1;$cnv<=$numval;$cnv++) 
									{
                                     echo '<a href="'.$pagi_url.(($cnv*$mulval*$pagi_perpage)-$pagi_perpage).'">'.($cnv*$mulval).'</a>';
									}*/ 
									?>
                                </div>-->
                        	<?php
