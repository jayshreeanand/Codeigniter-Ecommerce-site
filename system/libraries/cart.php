...
class CI_Cart {
   ...
   $this->_cart_contents['cart_total_no_discount'] = 0;
    $this->_cart_contents['cart_discount_percentage'] = 0;
		}
...
#$disc_perc = $this->_cart_contents[1]['options']['coupon'];
...
if($items >= $this->CI->config->item('discount_multiple_items'))
   ...
   $total_discount += ($price * $val['qty'] );
   $this->_cart_contents[$key]['price'] = $product_price;
   $this->_cart_contents[$key]['price_discount'] = $price;
   // Set the subtotal
   $this->_cart_contents[$key]['subtotal'] = ($this->_cart_contents[$key]['price'] * $this->_cart_contents[$key]['qty'] );
   $this->_cart_contents[$key]['subtotal_discount'] = ($this->_cart_contents[$key]['price_discount'] * $this->_cart_contents[$key]['qty'] );
   }
   else
   {   
   $total += ($product_price * $val['qty'] );
 ...
 $this->_cart_contents['cart_discount_percentage'] = $disc_perc;
  ...
  unset($cart['cart_total_no_discount']);
   unset($cart['cart_discount_percentage']);
		return $cart;
	}
 ...
 {
  return $this->_cart_contents['cart_discount_percentage'];
 }

 function update_options($couponcode)
    {
...
elseif ($couponcode =='10PERCENT')
{

$percentage =10;	
}
  elseif ($couponcode =='20PERCENT')
{

$percentage =20;	
}
elseif ($couponcode =='20PERCFB')
{

$percentage =20;	
}   
elseif ($couponcode =='10PERCFB')
{
...
else
{
 ...
 $this->_cart_contents['cart_discount_percentage'] =$percentage; 
 $save_cart = TRUE;
    ...
    // You can either update a single product using a one-dimensional array,
        ...
        // If it's not found we assume it's a multi-dimensional array
            ...
            #if ($this->_update_options($percentage) == TRUE)
            #{
             #   $save_cart = TRUE;
            #}
